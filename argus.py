...
def draw(stdscr, metrics):
    ...
    safe_addstr(stdscr, 1, 2, "ARGUS v0.3")

    safe_addstr(stdscr, 3, 2, f"CPU Usage:   {metrics['cpu']:5.1f}%")
    safe_addstr(stdscr, 4, 2, f"Memory:     {metrics['mem'].percent:5.1f}%")
    safe_addstr(stdscr, 5, 2, f"Disk (/):   {metrics['disk'].percent:5.1f}%")
    ...
    if ipv4:
        ...
        safe_addstr(stdscr, 8, 2, "IPv4: Not connected")
